    ...
    <form action="{{ route('admin.vehicles.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-6 space-y-6">
        ...
        <div>
            ...
            <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="features[]" value="{{ $feature->id }}" id="feature-{{ $feature->id }}" @checked(is_array(old('features')) && in_array($feature->id, old('features'))) class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div>
            ...
            <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="extras[]" value="{{ $extra->id }}" id="extra-{{ $extra->id }}" @checked(is_array(old('extras')) && in_array($extra->id, old('extras'))) class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                    <label for="extra-{{ $extra->id }}" class="ml-2 text-sm text-gray-700">{{ $extra->name }}</label>
                </div>
                @endforeach
            </div>
        </div>
