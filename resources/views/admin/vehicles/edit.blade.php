    ...
    <form action="{{ route('admin.vehicles.update', $vehicle->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="base_price_day" id="base_price_day"
                    value="{{ old('base_price_day', $vehicle->base_price_day) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div>
                ...
                <select name="transmission" id="transmission"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                    <option value="AT" @selected(old('transmission', $vehicle->transmission) == 'AT')>Automatic</option>
                    <option value="MT" @selected(old('transmission', $vehicle->transmission) == 'MT')>Manual</option>
                </select>
            </div>
            <div>
                <label for="fuel" class="block text-sm font-medium text-gray-700">Bahan Bakar</label>
                <select name="fuel" id="fuel" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                    required>
                    <option value="bensin" @selected(old('fuel', $vehicle->fuel) == 'bensin')>Bensin</option>
                    <option value="diesel" @selected(old('fuel', $vehicle->fuel) == 'diesel')>Diesel</option>
                    <option value="hybrid" @selected(old('fuel', $vehicle->fuel) == 'hybrid')>Hybrid</option>
                    <option value="ev" @selected(old('fuel', $vehicle->fuel) == 'ev')>Listrik</option>
                </select>
            </div>
            <div>
            ...
            <div>
                <label for="luggage" class="block text-sm font-medium text-gray-700">Bagasi</label>
                <input type="number" name="luggage" id="luggage" value="{{ old('luggage', $vehicle->luggage) }}"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('description', $vehicle->description) }}</textarea>
        </div>

        @php
            $vehicleFeatures = old('features', $vehicle->features->pluck('id')->toArray());
            $vehicleExtras = old('extras', $vehicle->extras->pluck('id')->toArray());
        @endphp
                ...
                @foreach ($features as $feature)
                    <div class="flex items-center">
                        <input type="checkbox" name="features[]" value="{{ $feature->id }}"
                            id="feature-{{ $feature->id }}" @checked(in_array($feature->id, $vehicleFeatures))
                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <label for="feature-{{ $feature->id }}"
                            class="ml-2 text-sm text-gray-700">{{ $feature->name }}</label>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <!-- Layanan Tambahan -->
        <div>
            <label class="block text-lg font-medium text-gray-900">Layanan Tambahan</label>
            <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach ($extras as $extra)
                    <div class="flex items-center">
                        <input type="checkbox" name="extras[]" value="{{ $extra->id }}"
                            id="extra-{{ $extra->id }}" @checked(in_array($extra->id, $vehicleExtras))
                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <label for="extra-{{ $extra->id }}"
                            class="ml-2 text-sm text-gray-700">{{ $extra->name }}</label>
                    </div>
                @endforeach
            </div>
        </div>
