        ...
        <nav class="mt-4 space-y-2 px-4">
                ...
                <div x-show="open" x-cloak class="mt-2 space-y-2 pl-6">
                    ...
                    @can('viewAny', \App\Models\Brand::class)
                        ...
                        <a href="{{ route('admin.extras.index') }}"
                            class="block rounded-lg py-2 px-4 text-sm {{ request()->routeIs('admin.extras.*') ? 'font-semibold text-white' : 'text-blue-200 hover:text-white' }}">Layanan
                            Tambahan</a>
                    @endcan
                </div>
            </div>
            ...
            @can('viewAny', \App\Models\User::class)
                ...
                <div x-data="{ open: {{ request()->routeIs(['admin.pages.*', 'admin.settings.*']) ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        class="flex w-full items-center justify-between rounded-lg py-2 px-4 transition-colors duration-200 
                        {{ request()->routeIs(['admin.pages.*', 'admin.settings.*']) ? 'bg-blue-600 font-semibold text-white' : 'text-blue-100 hover:bg-blue-600 hover:text-white' }}">
                        <div class="flex items-center">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div x-show="open" x-cloak class="mt-2 space-y-2 pl-6">
                        <a href="{{ route('admin.pages.index') }}"
                            class="block rounded-lg py-2 px-4 text-sm {{ request()->routeIs('admin.pages.*') ? 'font-semibold text-white' : 'text-blue-200 hover:text-white' }}">Halaman</a>
                        <a href="{{ route('admin.settings.index') }}"
                            class="block rounded-lg py-2 px-4 text-sm {{ request()->routeIs('admin.settings.*') ? 'font-semibold text-white' : 'text-blue-200 hover:text-white' }}">Situs
                            & Kontak</a>
                    </div>
                </div>
            @endcan
